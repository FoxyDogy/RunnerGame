...
public class CameraShake : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        m_seed = new Vector3(Random.Range(0.0f, kSeedRange), Random.Range(0.0f, kSeedRange),
            Random.Range(0.0f, kSeedRange));
    ...
    {
        var current = Speed * (Time.time * Vector3.one + m_seed);
        var adjustedMove = AdjustExtents(MoveExtents, Shake);
        var adjustedRotate = AdjustExtents(RotateExtents, Shake);

        var pos = Vector3.zero;
        if (MoveExtents != Vector3.zero) pos = ApplyNoise(current, adjustedMove);

        var rot = Quaternion.identity;
        if (RotateExtents != Vector3.zero) rot = Quaternion.Euler(ApplyNoise(current, adjustedRotate));
    ...
    private float GetT()
    {
        var t = Mathf.Clamp((Time.time - m_startTime) / Duration, 0.0f, 1.0f);
    ...
    private Vector3 AdjustExtents(Vector3 extents, ShakeType shakeType)
    {
        switch (shakeType)
        {
            ...
            case ShakeType.EaseInOut:
                return GetT() < 0.5f
                    ? AdjustExtents(extents, ShakeType.EaseIn)
                    : AdjustExtents(extents, ShakeType.EaseOut);
        }
    ...
    private Vector3 ApplyNoise(Vector3 target, Vector3 amplitude)
    {
        switch (Noise)
        {
            case NoiseType.Sin:
            {
                var x = amplitude.x * Mathf.Sin(target.x);
                var y = amplitude.y * Mathf.Sin(target.y);
                var z = amplitude.z * Mathf.Sin(target.z);
                return new Vector3(x, y, z);
            }
            case NoiseType.Perlin:
            {
                var x = amplitude.x * 2.0f * (Mathf.PerlinNoise(target.x, target.x) - 0.5f);
                var y = amplitude.y * 2.0f * (Mathf.PerlinNoise(target.y, target.y) - 0.5f);
                var z = amplitude.z * 2.0f * (Mathf.PerlinNoise(target.z, target.z) - 0.5f);
                return new Vector3(x, y, z);
            }
        }
